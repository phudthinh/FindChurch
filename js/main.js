...
function performSearchWithDefaultKeyword(searchKeyword, keyCity, keyDistrict) {
  navigator.geolocation.getCurrentPosition(function(position) {
    ...
    let searchURL = `https://api.mapbox.com/geocoding/v5/mapbox.places/nha tho ${searchKeyword}.json?access_token=${mapboxgl.accessToken}&bbox=${bbox}`;

    if(keyCity == "" && keyDistrict == "") {
        searchURL = `https://api.mapbox.com/geocoding/v5/mapbox.places/nha tho.json?access_token=${mapboxgl.accessToken}&bbox=${bbox}`;
    }

    else if (keyCity != "Chọn tỉnh/thành phố" && keyDistrict != "Chọn quận/huyện") {
        searchURL = `https://api.mapbox.com/geocoding/v5/mapbox.places/nha tho ${searchKeyword + " " + keyCity + " " + keyDistrict}.json?access_token=${mapboxgl.accessToken}&country=VN`;
    }
    else if (keyCity != "Chọn tỉnh/thành phố") {
        searchURL = `https://api.mapbox.com/geocoding/v5/mapbox.places/nha tho ${searchKeyword + " " + keyCity}.json?access_token=${mapboxgl.accessToken}&country=VN`;
    }
...
document.addEventListener('DOMContentLoaded', function() {
  performSearchWithDefaultKeyword("", "", "");
});
